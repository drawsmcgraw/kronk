...
Calls Ollama directly (localhost:11434) for accurate per-model timing.
...
SYSTEM_PROMPT = (
    ...
    "Do not use action text, emotes, or filler expressions like *winks* or *laughs*. "
...
PROMPTS = [
    ...
    },
    {
        ...
        "text": (
            "I have a FastAPI app. Add a POST /summarize endpoint that accepts "
            "{\"text\": \"...\", \"max_sentences\": 3} and returns {\"summary\": \"...\"}. "
            "Implement the summarization by extracting the first N sentences. "
            "Include input validation and a test using httpx.AsyncClient."
        ),
    },
]
...
def send_message(client: httpx.Client, text: str, model: str) -> dict:
    """Call Ollama /api/chat directly for accurate per-model timing."""
    result = {"response": "", "ttft_s": None, "generation_s": None, "error": None}
    messages = [
        ...
        with client.stream(
            ...
            for line in resp.iter_lines():
                ...
                if chunk.get("done"):
                    ...
                    result["ttft_s"] = round((t_first - t_start), 2) if t_first else None
                    result["generation_s"] = round((t_done - t_first), 2) if t_first else None
                    break
